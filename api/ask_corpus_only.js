...
// Dacă nu există context relevant în vector store: afișează un mesaj fix în limba utilizatorului.
...
// --- apel cu Responses API + file_search
async function askCorpusOnly({ question, lang }){
  ...
  const body = {
    ...
    input: [
      ...
      // îi spunem explicit CE mesaj fix să folosească la lipsa contextului
      { role: "user", content:
        `Language: ${lang}\n` +
    ...
    // intentionally no tool_choice here; instruction forces use; if nothing retrieved, it must refuse.
  };
...
// --- mică curățare de siguranță: scoatem eventuale prefixe nedorite
function cleanup(a){
  return String(a||"")
    .replace(/\bRO\s*[:\-]?\s*/gi,"")
    .replace(/\bEN\s*[:\-]?\s*/gi,"")
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    // Dacă totuși modelul n-a respectat (foarte rar), impunem noi refuzul:
    const hasRefusal = out.toUpperCase().includes(refusal(lang));
    const hasSources = /(^|\n)Surse\s*:/.test(out);
    if(!hasRefusal && !hasSources){
      // n-a listat surse și nici refuz => probabil fără context => forțăm refuzul
